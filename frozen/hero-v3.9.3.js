(()=>{
 ...
 // v3.9.3: reveal completion is a hard epoch boundary.
 // Everything before that moment is discarded. No quiet-time timer is used.
 // After the boundary, we ignore only the decaying tail inherited from gesture #1.
 // A renewed positive impulse is treated as gesture #2 and immediately becomes native scroll.
 ...
 function hardResetAtRevealEnd(){
   // Hard split: all momentum/velocity/input history from stage 1 is discarded.
   velocityEMA=0;
   lastInputAt=0;
 ...
 function isFreshDownwardImpulse(mag){
   // First post-boundary sample establishes the new zero-baseline and is never allowed through.
   ...
   // Inertial tail normally decays. A new physical swipe creates renewed acceleration.
   // Two rising samples, or one strong jump, define gesture #2 without using any silence timer.
 ...
 function startHandoff(){
   ...
   function tick(now){
     ...
     if(t<1){
       ...
       // THIS is the hard generation boundary requested:
       // no prior wheel timing, no prior velocity, no prior inertia state survives it.
 ...
 addEventListener('wheel',(e)=>{
   ...
   // While the automatic reveal is running, every wheel event belongs to stage 1 and is discarded.
   if(handoffRunning){
     e.preventDefault();
     return;
   }
   ...
   if(handoffDone){
     // Upward motion is explicit reverse intent: allow it immediately.
     if(dy<0){
       postRevealGuard=false;
       postRevealPrevMag=0;
       postRevealRiseCount=0;
     ...
     if(dy>0 && postRevealGuard){
       ...
       if(isFreshDownwardImpulse(mag)){
         ...
         postRevealPrevMag=0;
         postRevealRiseCount=0;
         // Do not preventDefault: the event that proves renewed intent becomes the first native scroll event.
         return;
       }
       // Still looks like inherited decaying momentum from stage 1; discard it.
       e.preventDefault();
       return;
     }

     // Guard already cleared: ordinary webpage scrolling in both directions.
     return;
   }
 ...
 addEventListener('touchstart',e=>{
   ...
   // Touch has a real new-contact boundary. After reveal, a new touchstart is gesture #2.
   if(handoffDone){
     postRevealGuard=false;
     postRevealPrevMag=0;
     postRevealRiseCount=0;
