(()=>{
 const track=document.getElementById('top'),art=document.getElementById('art'),copy=document.getElementById('heroCopy'),wash=document.getElementById('wash'),shade=document.getElementById('shade'),nav=document.getElementById('nav'),hint=document.getElementById('hint'),stage=document.getElementById('serviceStage'),services=document.getElementById('services');
 const clamp=(v,a=0,b=1)=>Math.max(a,Math.min(b,v));
 const smooth=t=>t*t*(3-2*t);
 const map=(p,a,b)=>smooth(clamp((p-a)/(b-a)));
 const mix=(a,b,t)=>a+(b-a)*t;

 // v3.9: the 340vh track produces exactly 2.4 viewport-heights of effective scroll.
 // The physical end of the track is the pure-white landing. Gesture #2 can re-enter immediately.
 const RESET_P=.90;
 const SLOW_HANDOFF_MS=1050;
 const FAST_HANDOFF_MS=820;
 const WHEEL_GESTURE_END_MS=220;
 const NEW_GESTURE_GAP_MS=85;
 ...
 function armGestureEnd(now=performance.now()){
   ...
   quarantineTimer=setTimeout(()=>{quarantine=false;},WHEEL_GESTURE_END_MS);
 }
 ...
 function render(){
   ...
   const forward=Math.pow(map(p,.018,.88),1.10);
   if(art){
     art.style.transform=`scale(${1.06+forward*.18})`;
     art.style.filter=`brightness(${1+forward*.22})`;
   }
   ...
   wash.style.opacity=map(p,.78,.94);
   ...
   const darkOut=map(p,.52,.79),lightIn=sv;
 ...
 function startHandoff(){
   ...
   function tick(now){
     ...
     handoff=smooth(t);
     schedule();
     if(t<1){handoffRaf=requestAnimationFrame(tick);}
     else{
       handoff=1;
 ...
 addEventListener('wheel',e=>{
   const dy=wheelPixels(e),m=metrics(),y=scrollY,now=performance.now();
   if(handoffRunning){
     const gap=lastBlockedWheelAt?now-lastBlockedWheelAt:0;
     if(dy>0&&gap>=NEW_GESTURE_GAP_MS)queuedSecondGesture=true;
     e.preventDefault();
     armGestureEnd(now);
     return;
   }
   ...
   // After the automatic reveal there is deliberately no gate in either direction.
   if(handoffDone)return;
   // Upward motion is never intercepted.
   if(dy<=0)return;
   // Gesture #1 can never overshoot the real white landing.
   if(y<m.endY-1){
     e.preventDefault();
     recordVelocity(dy);
 ...
 addEventListener('touchstart',e=>{
   ...
   if(handoffDone&&quarantine){quarantine=false;clearTimeout(quarantineTimer);}
 },{passive:true});
 addEventListener('touchmove',e=>{
   if(touchY==null)return;
   const nowY=e.touches[0]?.clientY??touchY,nowT=performance.now(),dy=touchY-nowY;
   touchY=nowY;
   const dt=Math.max(8,nowT-touchT);
   touchT=nowT;
   const m=metrics(),y=scrollY;
   if(handoffRunning||quarantine){e.preventDefault();return;}
   if(handoffDone)return;
   if(dy<=0)return;
   if(y<m.endY-1){
     e.preventDefault();
     ...
     scrollTo(0,next);
     schedule();
     if(next>=m.endY-1){quarantine=true;startHandoff();}
   }else{
     e.preventDefault();
     quarantine=true;
     startHandoff();
 ...
 addEventListener('touchend',()=>{touchY=null;touchGestureEnded();},{passive:true});
 addEventListener('touchcancel',()=>{touchY=null;touchGestureEnded();},{passive:true});
 schedule();
})();